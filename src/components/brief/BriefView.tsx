...
export function BriefView({ progress, onProgressChange, initialAnswers }: BriefViewProps) {
  ...
  return (
    <section className="relative flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-paper">
      <div className="pointer-events-none absolute inset-0 z-0 bg-[radial-gradient(ellipse_90%_70%_at_60%_-5%,rgba(255,77,158,0.1)_0%,transparent_55%)]" />

      <div className="preview-scroll relative z-10 flex min-h-0 flex-1 flex-col overflow-y-auto">
        <div
          ...
          >
            {!showResult && (
              <div className="mb-6 shrink-0 lg:mb-8">
                ...
                <div className="mt-3 h-px w-full overflow-hidden rounded-full bg-accent-soft">
                  <motion.div
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <h2 className="font-display mt-3 text-[1.75rem] leading-[1.08] tracking-[-0.03em] text-ink lg:text-[2.35rem]">
                  {headline}
                </h2>
                {!showResult && step === 0 && (
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div className="rounded-2xl border border-black/12 bg-accent-soft p-5 backdrop-blur-sm">
                        ...
                        <p className="mt-2 text-[12px] text-muted">{caseStudy.category}</p>
                        <p className="mt-3 text-[13px] leading-relaxed text-muted">
                          {caseStudy.description}
                        </p>
                      </div>

                      <div className="rounded-2xl border border-accent/20 bg-accent-soft p-5 backdrop-blur-sm">
                    ...
                    <label className="block">
                      <span className="text-[12px] text-muted">{briefCopy.notesLabel}</span>
                      <textarea
                        value={answers.notes}
                        onChange={(event) =>
                        ...
                        rows={3}
                        className="mt-2 w-full resize-none rounded-xl border border-black/12 bg-paper px-4 py-3 text-[13px] text-ink outline-none transition placeholder:text-faint focus:border-accent/40"
                      />
                    </label>
                  </div>
                  ) : (
      ...
      <footer className="relative z-10 shrink-0 border-t border-black/10 bg-paper px-5 pt-5 pb-safe-8 lg:px-10 lg:pt-6 lg:pb-safe-10">
        <div className="mx-auto flex max-w-2xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={handleBack}
            disabled={!showResult && step === 0}
            className="rounded-full px-4 py-2.5 text-[12px] text-muted transition enabled:hover:text-muted disabled:opacity-30"
          >
            {briefCopy.back}
          </button>
          ...
          {showResult ? (
            <div className="flex items-center gap-2">
              <button
                ...
                className="rounded-full border border-black/12 px-4 py-2.5 text-[12px] text-muted transition hover:border-black/15 hover:text-muted"
              >
                {briefCopy.restart}
              </button>
              <a
            ...
            <button
              ...
              className="btn-solid !text-[12px] disabled:opacity-40"
            >
...
function OptionCard({ selected, label, description, onClick, compact }: OptionCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`project-tile w-full ${compact ? "px-4 py-3.5" : "px-4 py-4"} ${
        selected ? "border-accent ring-2 ring-accent/15" : ""
      }`}
    >
      <p className="text-[13px] font-medium text-ink">{label}</p>
      <p className="mt-1.5 text-[12px] leading-relaxed text-muted group-hover:text-muted">
        {description}
      </p>
    </button>
  );
